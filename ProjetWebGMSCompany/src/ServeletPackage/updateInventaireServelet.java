package ServeletPackage;

import java.io.IOException;
import java.sql.SQLException;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import org.json.JSONArray;
import org.json.JSONObject;
import DAO.inventaireProduitDAO;

/**
 * Servlet implementation class updateInventaire
 */
@WebServlet("/updateInventaireServelet")
public class updateInventaireServelet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public updateInventaireServelet() {
        super();
        // TODO Auto-generated constructor stub
    }
    inventaireProduitDAO invDAO = new inventaireProduitDAO();
    String pageOutput = "/ShopServelet";

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		int qteEnstock = Integer.parseInt(request.getParameter("qteEnStock"));
		int seuilMin = Integer.parseInt(request.getParameter("seuilMin"));
		int qteAachete = Integer.parseInt(request.getParameter("qteAachete"));
		int produitID = Integer.parseInt(request.getParameter("produitID"));
		System.out.println("produitID "+ produitID);
		try {
			invDAO.UpdateInventaireProduit(produitID, qteEnstock, seuilMin, qteAachete);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		response.getWriter().append("Served at: ").append(request.getContextPath());
		 RequestDispatcher dispatcher =
					getServletContext().getRequestDispatcher(pageOutput);
					 dispatcher.forward(request, response); 
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

}
